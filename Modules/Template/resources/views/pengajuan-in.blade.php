  ...
  <table id="pengajuanTable" class="table table-hover table-borderless mb-0">
    ...
    <tbody>
      @foreach ($pengajuans as $index => $pengajuan)
        <tr>
          <td>{{ $index + 1 }}</td>
          <td>{{ $pengajuan->mahasiswa->nim ?? 'N/A' }}</td>
          <td>{{ $pengajuan->name }}</td>
          <td>{{ $pengajuan->email }}</td>
          <td>{{ $pengajuan->role }}</td>
          <td class="text-end">{{ $pengajuan->created_at->format('d M Y') }}</td>

          <td class="text-end">
            <a href="{{ route('edit.mahasiswa', $pengajuan->id) }}" class="btn btn-warning btn-sm">
              Edit
            </a>

            <form action="{{ route('hapus.mahasiswa', $pengajuan->id) }}"
                  method="POST"
                  class="d-inline">
              @csrf
              @method('DELETE')
